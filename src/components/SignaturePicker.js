...
export default function SignaturePicker({ nameOptions, onSignature, onClose, defaultMethod }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in slide-in-from-bottom-8 duration-300 border border-black/5" onClick={(e) => e.stopPropagation()}>

        {/* Header */}
        <div className="px-5 py-4 border-b border-gray-100 dark:border-neutral-700 flex items-center justify-between">
          <div className="flex items-center gap-2">
            {mode !== 'choose' && (
              <button onClick={() => setMode('choose')} className="p-1 -ml-1 text-gray-400 hover:text-gray-600 dark:hover:text-neutral-300 transition-colors">
          ...
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-neutral-300 transition-colors">
          ...
          <div className="p-5 space-y-3">
            <button onClick={() => setMode('draw')}
              ...
              <div className="w-10 h-10 rounded-lg bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                <svg className="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-gray-900 dark:text-neutral-100">Draw Signature</div>
                <div className="text-xs text-gray-400 dark:text-neutral-500 mt-0.5">Use your finger or mouse to sign</div>
              </div>
              <svg className="w-4 h-4 text-gray-300 dark:text-neutral-600 group-hover:text-gray-400 group-hover:translate-x-0.5 transition-all" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
            </button>

            <button onClick={() => {
                const selected = nameOptions?.find(n => n.id === selectedNameId);
                setCursiveText(selected?.value || ''); setMode('cursive');
              }}
              ...
              <div className="w-10 h-10 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                <svg className="w-5 h-5 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-gray-900 dark:text-neutral-100">Use Full Name</div>
                <div className="text-xs text-gray-400 dark:text-neutral-500 mt-0.5">Auto-render a name in cursive style</div>
              </div>
              <svg className="w-4 h-4 text-gray-300 dark:text-neutral-600 group-hover:text-gray-400 group-hover:translate-x-0.5 transition-all" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
            </button>

            <button onClick={() => { setCursiveText(''); setMode('cursive'); }}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-400 dark:text-neutral-500 mt-0.5">Type any text and see it in cursive</div>
              </div>
              <svg className="w-4 h-4 text-gray-300 dark:text-neutral-600 group-hover:text-gray-400 group-hover:translate-x-0.5 transition-all" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
            </button>
          </div>
          ...
          <div className="p-5">
            ...
            <div className="flex justify-between mt-3">
              <button onClick={() => sigRef.current?.clear()}
                className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-neutral-300 font-medium transition-colors">Clear</button>
          ...
          <div className="p-5 space-y-4">
            <div className="bg-gray-50 dark:bg-neutral-900 rounded-xl border border-gray-200 dark:border-neutral-700 p-4 flex items-center justify-center min-h-[100px]">
              {cursiveText.trim() ? (
                ...
                <span className="text-sm text-gray-300 dark:text-neutral-600">Your signature preview will appear here</span>
              )}
            </div>
            {nameOptions?.length > 0 && (
              <div>
                <label className="block text-xs text-gray-400 dark:text-neutral-500 mb-1.5 font-medium">Use field value</label>
                <select value={selectedNameId || ''} onChange={(e) => {
                  const selected = nameOptions.find(n => n.id === e.target.value);
                  if (selected) { setSelectedNameId(selected.id); setCursiveText(selected.value); }
                }}
                  ...
                  {nameOptions.map((n) => (
                    <option key={n.id} value={n.id}>{n.label || n.id}: {n.value}</option>
                  ))}
                </select>
              </div>
            )}
            <div>
              <label className="block text-xs text-gray-400 dark:text-neutral-500 mb-1.5 font-medium">Or type your own</label>
              <input
                autoFocus
                type="text"
                value={cursiveText}
        ...
        <div className="px-5 py-4 border-t border-gray-100 dark:border-neutral-700 flex justify-end gap-3">
          {mode === 'choose' ? (
            <button onClick={onClose}
              className="px-5 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-700 transition-all">Cancel</button>
          ) : mode === 'draw' ? (
            <>
              <button onClick={() => setMode('choose')}
                className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-700 transition-all">Back</button>
              <button onClick={handleDrawSave}
                className="px-6 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-all active:scale-[0.98]">Use Signature</button>
            </>
          ) : (
            <>
              <button onClick={() => setMode('choose')}
                className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-700 transition-all">Back</button>
              <button onClick={handleCursiveConfirm} disabled={!cursiveText.trim()}
                className="px-6 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-40 transition-all active:scale-[0.98]">Use Signature</button>
            </>
          )}
        </div>
      </div>
