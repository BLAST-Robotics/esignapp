...
function setToken(t) {
  try { if (t) localStorage.setItem('ks_token', t); else localStorage.removeItem('ks_token'); } catch {}
}
...
export default function AdminClient() {
  ...
  const apiReq = useCallback(async (url, options = {}) => {
    ...
    if (res.status === 401) { setUser(null); setToken(''); throw new Error('Session expired'); }
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try { const r = await apiReq('/api/documents'); setDocuments((await r.json()).documents); }
  ...
  useEffect(() => {
    if (!user) return;
    apiReq('/api/admin/signatures').then((r) => r.json()).then((d) => setAllSignatures(d.signatures || [])).catch(() => {});
  }, [user, apiReq]);

  const fetchUsers = useCallback(async () => {
    try { const r = await apiReq('/api/admin/users'); setAllUsers((await r.json()).users || []); }
    catch {}
  }, [apiReq]);
  ...
  const handleLogout = () => {
    ...
    setToken(''); setUser(null); setDocuments([]);
  };
  ...
  const handleDelete = async (id) => {
    setError(null);
    try { await apiReq(`/api/documents/${id}`, { method: 'DELETE' }); setDeleteId(null); fetchDocs(); }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-neutral-950 transition-colors duration-300">
      <header className="bg-white/90 dark:bg-neutral-900/90 backdrop-blur-md border-b border-gray-200/80 dark:border-neutral-800 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {user.role === 'admin' && <span className="text-[10px] font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded-full">Admin</span>}
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400 dark:text-neutral-500 hidden sm:inline">{user.email}</span>
            <ThemeToggle />
            <button onClick={handleLogout} className="text-sm text-gray-400 hover:text-gray-600 dark:text-neutral-500 dark:hover:text-neutral-300 transition-colors flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" /></svg>
              Sign Out
            </button>
          </div>
        </div>
        {user.role === 'admin' && (
          <div className="border-t border-gray-100 dark:border-neutral-800">
            <div className="max-w-7xl mx-auto px-4 flex gap-1 py-2">
              <button onClick={() => setDashboardTab('documents')}
              ...
              <button onClick={() => setDashboardTab('users')}
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {error && (
          <div className="mb-4 px-4 py-3 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-xl text-sm">{error}</div>
        )}
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-xl border border-gray-200/80 dark:border-neutral-800 overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-neutral-800 flex items-center justify-between">
            ...
            <label className={`px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-all cursor-pointer flex items-center gap-2 shadow-sm ${uploading ? 'opacity-40 pointer-events-none' : ''}`}>
              {uploading ? 'Uploading...' : (
                <><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>Upload PDF</>
              )}
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {documents.map((doc) => (
                    <tr key={doc.id} className="border-b border-gray-100 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800/30 cursor-pointer"
                      onClick={() => { if (editingId !== doc.id) window.location.href = `/admin/documents/${doc.id}`; }}>
                      <td className="px-5 py-4" onClick={(e) => { if (editingId === doc.id) e.stopPropagation(); }}>
                        {editingId === doc.id ? (
                          <input autoFocus value={editValue} onChange={(e) => setEditValue(e.target.value)}
                            onBlur={async () => {
                              if (editValue.trim()) {
                                try { await apiReq(`/api/documents/${doc.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: editValue.trim() }) }); fetchDocs(); } catch {}
                              }
                              setEditingId(null);
                            }}
                          ...
                          <div className="text-gray-900 dark:text-neutral-100 font-medium group flex items-center gap-1.5"
                            onDoubleClick={() => { setEditingId(doc.id); setEditValue(doc.title); }}>
                            {doc.title}
                            {doc.permission && (
                              <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-full ${doc.permission === 'manage' ? 'bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400' : doc.permission === 'edit' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-gray-50 text-gray-500 dark:bg-neutral-800 dark:text-neutral-400'}`}>
                                {doc.permission}
                              </span>
                            )}
                            <svg className="w-3.5 h-3.5 text-gray-300 dark:text-neutral-600 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                          </div>
                        )}
                        <div className="text-xs text-gray-400 mt-0.5 sm:hidden">{doc.status} &middot; {doc.signature_count || 0} sigs</div>
                      </td>
                      <td className="px-4 py-4 hidden sm:table-cell">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${doc.status === 'active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 text-gray-500 dark:bg-neutral-800 dark:text-neutral-400'}`}>
                          {doc.status}
                        </span>
                      </td>
                      ...
                      <tr key={u.id} className="border-b border-gray-100 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800/30">
                        ...
                        <td className="px-4 py-4">
                          {editUserId === u.id ? (
                            ...
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${u.role === 'admin' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-gray-50 text-gray-500 dark:bg-neutral-800 dark:text-neutral-400'}`}>
                              {u.role}
                            </span>
                          )}
                        </td>
                          ...
                          {editUserId === u.id ? (
                            <div className="flex items-center gap-1 justify-end">
                              <button onClick={async () => {
                                try {
                                  await apiReq(`/api/admin/users/${u.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editUserData) });
                                  setEditUserId(null);
                                  fetchUsers();
                                } catch (e) { setError(e.message); }
                              }} className="px-2 py-1 text-xs text-blue-600 hover:text-blue-700 font-medium">Save</button>
                              <button onClick={() => { setEditUserId(null); fetchUsers(); }} className="px-2 py-1 text-xs text-gray-400 hover:text-gray-600">Cancel</button>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 justify-end">
                              <button onClick={() => { setEditUserId(u.id); setEditUserData({ email: u.email, name: u.name || '', role: u.role }); }}
                              ...
                              <button onClick={async () => {
                                ...
                                try { await apiReq(`/api/admin/users/${u.id}`, { method: 'DELETE' }); fetchUsers(); }
                                catch (e) { setError(e.message); }
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setDeleteId(null)}>
          <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl border border-gray-200 dark:border-neutral-700 w-full max-w-sm overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-5 space-y-3">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-neutral-100">Delete Document</h3>
              <p className="text-sm text-gray-500 dark:text-neutral-400">Are you sure? This will also remove all signatures for this document.</p>
            </div>
            <div className="px-6 py-4 border-t border-gray-100 dark:border-neutral-700 flex justify-end gap-3">
              <button onClick={() => setDeleteId(null)} className="px-4 py-2 text-sm text-gray-600 dark:text-neutral-400 hover:text-gray-800 dark:hover:text-neutral-200 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors">Cancel</button>
              <button onClick={() => handleDelete(deleteId)} className="px-5 py-2 bg-red-600 text-white rounded-xl text-sm font-medium hover:bg-red-700 transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}
