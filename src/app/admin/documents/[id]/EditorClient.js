...
function getToken() {
  ...
  try { return localStorage.getItem('ks_token') || ''; } catch { return ''; }
}
...
function DraggableField({ field, scale, selected, onSelect, onUpdate }) {
  ...
  useEffect(() => {
    ...
    return () => { el.removeEventListener('pointerdown', down); document.removeEventListener('pointermove', move); document.removeEventListener('pointerup', up); };
  ...
  useEffect(() => {
    ...
    function down(e) {
      ...
      const d = dragRef.current;
      startX = e.clientX; startY = e.clientY;
      startRW = d.w; startRH = d.h;
      startRX = d.x; startRY = d.y;
      document.addEventListener('pointermove', move);
      document.addEventListener('pointerup', up);
    }
    function move(e) {
      ...
      if (h.includes('w')) { d.w = Math.max(50, startRW - dx); d.x = startRX + dx; }
      if (h.includes('s')) d.h = Math.max(30, startRH + dy);
      if (h.includes('n')) { d.h = Math.max(30, startRH - dy); d.y = startRY + dy; }
      el.style.width = `${d.w * scale}px`; el.style.height = `${d.h * scale}px`;
      el.style.left = `${d.x * scale}px`; el.style.top = `${d.y * scale}px`;
    }
    function up() {
      document.removeEventListener('pointermove', move);
      document.removeEventListener('pointerup', up);
  ...
  const H = 8;
  return (
    <div ref={elRef} data-field-id={field.id} onPointerDown={() => onSelect(field.id)}
      style={{
        position: 'absolute', left: `${(field.x || 0) * scale}px`, top: `${(field.y || 0) * scale}px`,
        width: `${(field.width || 200) * scale}px`, height: `${(field.height || 40) * scale}px`,
        ...
        fontWeight: selected ? 600 : 400, color: selected ? '#fff' : '#000',
        cursor: 'grab', zIndex: selected ? 10 : 1, overflow: 'visible', padding: '2px',
      ...
      {selected && (
        <>
          <span data-handle="nw" style={{ position: 'absolute', top: `-${H/2}px`, left: `-${H/2}px`, width: `${H}px`, height: `${H}px`, backgroundColor: '#fff', border: '2px solid #000', borderRadius: '50%', cursor: 'nwse-resize', zIndex: 20 }} />
          <span data-handle="ne" style={{ position: 'absolute', top: `-${H/2}px`, right: `-${H/2}px`, width: `${H}px`, height: `${H}px`, backgroundColor: '#fff', border: '2px solid #000', borderRadius: '50%', cursor: 'nesw-resize', zIndex: 20 }} />
          <span data-handle="sw" style={{ position: 'absolute', bottom: `-${H/2}px`, left: `-${H/2}px`, width: `${H}px`, height: `${H}px`, backgroundColor: '#fff', border: '2px solid #000', borderRadius: '50%', cursor: 'nesw-resize', zIndex: 20 }} />
          <span data-handle="se" style={{ position: 'absolute', bottom: `-${H/2}px`, right: `-${H/2}px`, width: `${H}px`, height: `${H}px`, backgroundColor: '#fff', border: '2px solid #000', borderRadius: '50%', cursor: 'nwse-resize', zIndex: 20 }} />
        </>
      )}
    </div>
  );
...
function CtxFieldEditor({ ctxFieldId, ctxNewPos, ctxPos, ctxRef, fields, numPages, selectedId, setCtxPos, setCtxFieldId, setCtxNewPos, updateField, addField, removeField, canEdit }) {
  ...
  useEffect(() => {
    ...
    function rdown(e) {
      if (!e.target.dataset?.resize) return;
      startX = e.clientX; startY = e.clientY;
      startSW = sizeRef.current.w; startSH = sizeRef.current.h;
    ...
    return () => { el.removeEventListener('pointerdown', rdown); document.removeEventListener('pointermove', rmove); document.removeEventListener('pointerup', rup); };
  }, []);

  function close() { setCtxPos(null); setCtxFieldId(null); setCtxNewPos(null); }

  return (
    <div ref={ctxRef} style={{ position: 'fixed', left: ctxPos.x, top: ctxPos.y, zIndex: 1000, width: 320 }}
      className="bg-neutral-900 border border-neutral-700 rounded-xl shadow-2xl overflow-hidden">
      <div data-ctxheader className="px-4 py-2.5 bg-neutral-800 flex items-center justify-between cursor-grab select-none" style={{ touchAction: 'none' }}>
        <span className="text-xs font-medium text-neutral-300 flex items-center gap-2">
          <svg className="w-3 h-3 text-neutral-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
          {isNew ? 'New Field' : f?.label || 'Edit Field'}
        </span>
        <button data-ctxclose onClick={close} className="p-0.5 text-neutral-500 hover:text-neutral-300 transition-colors">
      ...
      {isNew ? (
        <div className="p-5 text-center space-y-3">
          ...
          <button onClick={() => { addField(ctxNewPos); close(); }}
            className="w-full px-4 py-2 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 transition-all">Create Field</button>
          <button onClick={close}
            className="w-full px-4 py-2 text-xs text-neutral-400 hover:text-neutral-200 rounded-lg hover:bg-neutral-800 transition-all">Cancel</button>
        </div>
      ) : f ? (
        <>
          <div className="p-4 space-y-2 overflow-y-auto" style={{ maxHeight: '60vh' }}>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium">Label</label>
                <input type="text" value={f.label} onChange={(e) => updateField({ ...f, label: e.target.value })}
                  className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit} />
              </div>
              <div>
                <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium">Type</label>
                <select value={f.field_type} onChange={(e) => updateField({ ...f, field_type: e.target.value })}
                  className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit}>
            ...
            <div className="grid grid-cols-4 gap-2">
              {['x','y','width','height'].map((prop) => (
                <div key={prop}>
                  <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium uppercase">{prop}</label>
                  <input type="number" value={f[prop] ?? 0} onChange={(e) => updateField({ ...f, [prop]: Number(e.target.value) })}
                    className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit} />
                </div>
              ))}
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium">Font Size</label>
                <input type="number" value={f.font_size || 14} onChange={(e) => updateField({ ...f, font_size: Number(e.target.value) })}
                  className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit} />
              </div>
              <div>
                <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium">Page</label>
                <select value={f.page_number || 0} onChange={(e) => updateField({ ...f, page_number: Number(e.target.value) })}
                  className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit}>
                  {Array.from({ length: numPages || 1 }, (_, i) => (
                    <option key={i + 1} value={i + 1}>Page {i + 1}{numPages && i + 1 === numPages ? ' (last)' : ''}</option>
                  ))}
                </select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {f.field_type === 'signature' && (
                <div>
                  <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium">Default Method</label>
                  <select value={f.default_method || 'draw'} onChange={(e) => updateField({ ...f, default_method: e.target.value })}
                    className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit}>
              ...
              {f.field_type === 'date' && (
                <div>
                  <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium">Date Format</label>
                  <select value={f.date_format || 'signing'} onChange={(e) => updateField({ ...f, date_format: e.target.value })}
                    className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit}>
            ...
            {/* Field validation */}
            <div>
              <label className="block text-[10px] text-neutral-500 mb-0.5 font-medium">Validation</label>
              <select value={f.validation || 'required'} onChange={(e) => updateField({ ...f, validation: e.target.value })}
                className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500" disabled={!canEdit}>
                {VALIDATION_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <button onClick={() => { removeField(f.id); close(); }}
              className="w-full mt-1 px-3 py-2 text-xs text-red-400 hover:text-red-300 hover:bg-red-900/30 rounded-lg transition-all">Delete Field</button>
          </div>
          <div className="px-4 py-3 border-t border-neutral-700 flex items-center justify-between">
            <button onClick={close}
              className="px-4 py-1.5 text-xs text-neutral-400 hover:text-neutral-200 rounded-lg hover:bg-neutral-800 transition-all">Cancel</button>
            <button onClick={close}
              className="px-5 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 transition-all shadow-sm">Done</button>
          </div>
          <div data-resize style={{ position: 'absolute', right: '-3px', bottom: '-3px', width: '12px', height: '12px', cursor: 'nwse-resize', zIndex: 10 }}>
            <svg className="w-3 h-3 text-neutral-500" viewBox="0 0 12 12" fill="currentColor"><path d="M12 0v12H0l12-12z" /></svg>
          </div>
        </>
      ) : null}
    </div>
...
export default function DocumentEditor({ documentId }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/documents/${documentId}`, { headers: authHeaders })
      .then((r) => {
        if (r.status === 401) { router.push('/admin'); throw new Error('Unauthorized'); }
        return r.json();
      })
      .then((data) => {
  ...
  const fetchSignatures = useCallback(async () => {
    ...
    } catch (e) { console.error(e); } finally { setLoadingSigs(false); }
  }, [documentId]);
  ...
  useEffect(() => {
    ...
    function dismiss(e) {
      if (ctxRef.current && !ctxRef.current.contains(e.target)) {
        setCtxPos(null); setCtxFieldId(null); setCtxNewPos(null);
      }
    }
  ...
  const updateField = useCallback((updated) => {
    setFields((prev) => prev.map((f) => f.id === updated.id ? { ...f, ...updated, width: updated.width, height: updated.height } : f));
    setSelectedId(updated.id);
  }, []);

  function handlePageContext(e, pn) {
    e.preventDefault();
    const modalW = 320, modalH = 400;
  ...
  function addField(pos) {
    ...
    setFields((prev) => [...prev, { id, x, y, width: 250, height: 40, font_size: 14, label: 'New Field', field_type: 'name', required: true, validation: 'required', page_number: pageNum }]);
    setSelectedId(id);
    setCtxFieldId(id);
    setCtxNewPos(null);
  ...
  async function saveFields() {
    ...
    try {
      ...
      await fetch(`/api/documents/${documentId}`, {
        method: 'PUT', headers: { ...authHeaders, 'Content-Type': 'application/json' }, body: JSON.stringify({ title, slug, collect_email: collectEmail }),
      });
      setMessage({ type: 'success', text: 'Saved!' });
    } catch (e) { setMessage({ type: 'error', text: e.message }); } finally { setSaving(false); }
  }
  ...
  async function downloadSig(id) {
    ...
    } catch { setMessage({ type: 'error', text: 'Download failed' }); } finally { setDownloadingId(null); }
  }
  ...
  async function saveEdit() {
    ...
    try {
      const r = await fetch(`/api/admin/signatures/${editSig.id}`, {
        method: 'PUT', headers: { ...authHeaders, 'Content-Type': 'application/json' }, body: JSON.stringify({ field_values: editValues }),
      });
      if (!r.ok) throw new Error('Failed');
  ...
  async function downloadAll() {
    ...
    try {
      ...
      const blob = await r.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a'); a.href = url; a.download = `signed-${doc?.slug || documentId}.zip`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch { setMessage({ type: 'error', text: 'Failed to download all' }); } finally { setDownloadingAll(false); }
  }
  ...
  async function addShare() {
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) { setMessage({ type: 'error', text: data.error || 'Share failed' }); return; }
      setShareEmail('');
      setMessage({ type: 'success', text: 'Shared!' });
      loadPermissions();
    } catch { setMessage({ type: 'error', text: 'Share failed' }); } finally { setShareLoading(false); }
  }

  async function removeShare(email) {
    try {
      await fetch(`/api/documents/${documentId}/permissions?email=${encodeURIComponent(email)}`, { method: 'DELETE', headers: authHeaders });
      loadPermissions();
    } catch {}
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-neutral-950 transition-colors duration-300">
      <header className="bg-white/90 dark:bg-neutral-900/90 backdrop-blur-md border-b border-gray-200/80 dark:border-neutral-800 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/admin')} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
            ...
            <div className="flex items-center gap-1.5">
              <input value={title} onChange={(e) => setTitle(e.target.value)}
                className="text-base font-semibold text-gray-900 dark:text-gray-100 bg-transparent border-none outline-none focus:ring-0" disabled={!canEdit} />
              <svg className="w-3.5 h-3.5 text-gray-300 dark:text-neutral-600" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
            </div>
            {permission && (
              <span className="text-[10px] font-medium text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30 px-2 py-0.5 rounded-full">{permission}</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {canManage && (
              <button onClick={() => setShowShare(true)}
                ...
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>
                Share
              </button>
            )}
            <ThemeToggle />
            {canManage && (
              <label className="px-3 py-2 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 border border-gray-200 dark:border-neutral-700 rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-800 transition-all cursor-pointer">
                ...
                <input type="file" accept=".pdf" className="hidden" onChange={async (e) => {
                  ...
                  fd.append('file', file);
                  try {
                    const r = await fetch(`/api/documents/${documentId}`, { method: 'PUT', headers: { Authorization: `Bearer ${token}` }, body: fd });
                    if (r.ok) { setMessage({ type: 'success', text: 'PDF replaced!' }); setNumPages(null); }
                    else throw new Error('Failed');
                  } catch { setMessage({ type: 'error', text: 'Failed to replace PDF' }); }
                  e.target.value = '';
                }} />
              </label>
            )}
      ...
      <div className="sticky top-14 z-10 bg-white/90 dark:bg-neutral-950/90 backdrop-blur-md border-b border-gray-200/80 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-3">
          <div className="bg-white dark:bg-neutral-900 rounded-xl border border-gray-200/80 dark:border-neutral-800 p-1 flex shadow-sm" role="tablist">
            <button role="tab" aria-selected={tab === 'fields'} onClick={() => setTab('fields')}
            ...
            <button role="tab" aria-selected={tab === 'signatures'} onClick={() => setTab('signatures')}
          ...
          <div className="flex items-center gap-2">
            <div className="bg-white dark:bg-neutral-900 rounded-xl border border-gray-200/80 dark:border-neutral-800 p-2 flex items-center gap-1.5 shadow-sm">
              ...
              <input value={slug} onChange={(e) => setSlug(e.target.value)}
                className="w-24 text-[11px] text-gray-700 dark:text-gray-300 bg-transparent border-none outline-none focus:ring-0 p-0" placeholder="friendly-url" disabled={!canEdit} />
            </div>
            <div className="bg-white dark:bg-neutral-900 rounded-xl border border-gray-200/80 dark:border-neutral-800 p-2 flex items-center gap-1.5 shadow-sm max-w-[180px]">
              <svg className="w-3 h-3 text-gray-400 shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.57a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364L5.1 12" /></svg>
              <input readOnly value={friendlyLink} onClick={(e) => e.target.select()} className="flex-1 text-[11px] text-gray-500 bg-gray-50 dark:bg-neutral-800 px-1.5 py-1 rounded border border-gray-200 dark:border-neutral-700 focus:outline-none select-all min-w-0" />
              <button onClick={() => { copyToClipboard(friendlyLink); setMessage({ type: 'success', text: 'Copied!' }); }}
                className="text-[10px] text-blue-600 hover:text-blue-700 dark:hover:text-blue-400 font-medium whitespace-nowrap shrink-0">Copy</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div ref={pdfRef} className="bg-neutral-900 rounded-xl border border-neutral-800 overflow-hidden shadow-sm">
              <div className="px-4 py-2.5 border-b border-neutral-800 bg-neutral-800/50 flex items-center justify-between">
                <span className="text-xs font-medium text-gray-400 flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />Right-click fields or empty area &mdash; drag to reposition
                </span>
                <span className="text-[10px] text-gray-400 font-mono">{scale.toFixed(2)}x</span>
              </div>
              <Document file={`/api/public/document/${documentId}/pdf`}
                onLoadSuccess={({ numPages: n }) => { setNumPages(n); setPdfDims({ width: 612, height: 792 }); }}
                ...
                <div className="flex flex-col items-center p-2" style={{backgroundColor: '#262626'}}>
                  {numPages && Array.from({ length: numPages }, (_, i) => {
                    const pn = i + 1;
                    const pageFields = fields.filter((f) => (f.page_number || 0) === 0 ? pn === numPages : (f.page_number || 0) === pn);
                    return (
                      <div key={pn} data-page={pn} className="relative mb-4 last:mb-0 rounded-lg overflow-hidden" style={{ width: pdfWidth, backgroundColor: '#404040' }}>
                        <Page pageNumber={pn} renderTextLayer={false} renderAnnotationLayer={false}
                          width={pdfWidth} />
                        <div className="absolute inset-0 pointer-events-auto" style={{ width: pdfWidth }} onContextMenu={(e) => handlePageContext(e, pn)}>
                          {pageFields.map((f) => (
                            <DraggableField key={`${f.id}-${f.page_number || 0}`} field={f} scale={scale} selected={selectedId === f.id} onSelect={setSelectedId} onUpdate={updateField} />
                          ))}
                        </div>
                      </div>
                    );
              ...
              <div className="flex items-center gap-2">
                {canEdit && (
                  <button onClick={() => addField()}
                    ...
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>Add Field
                  </button>
                )}
                <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none flex-1">
                  ...
                  {fields.map((f) => {
                    ...
                    return (
                      <button key={f.id} onClick={() => openFieldEdit(f.id)}
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${isSel ? 'bg-white/70' : ''}`} style={{ backgroundColor: isSel ? 'rgba(255,255,255,0.7)' : c.border }} />
                        {f.label || f.field_type}
                        {canEdit && (
                          <span role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); removeField(f.id); }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); removeField(f.id); } }}
            ...
            <div className="bg-white dark:bg-neutral-900 rounded-xl border border-gray-200/80 dark:border-neutral-800 overflow-hidden shadow-sm">
              {loadingSigs ? (
                <div className="flex items-center justify-center py-16 text-gray-400 text-sm">
                  <svg className="animate-spin h-5 w-5 mr-2" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Loading&hellip;
                </div>
              ) : signatures.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50 dark:bg-neutral-800/50 border-b border-gray-200 dark:border-neutral-800">
                        {fields.map((f) => (
                          <th key={f.id} className="text-left px-3 py-3 font-medium text-gray-600 dark:text-neutral-400 text-xs">{f.label}</th>
                        ))}
                        <th className="text-left px-3 py-3 font-medium text-gray-600 dark:text-neutral-400 text-xs hidden sm:table-cell">Signed At</th>
                        <th className="text-left px-3 py-3 font-medium text-gray-600 dark:text-neutral-400 text-xs hidden lg:table-cell">IP</th>
                        <th className="text-right px-3 py-3 font-medium text-gray-600 dark:text-neutral-400 text-xs">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {signatures.map((sig) => (
                        <tr key={sig.id} className="border-b border-gray-100 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800">
                          {fields.map((f) => {
                            const val = sig.field_values?.[f.id] || '';
                            return (
                              <td key={f.id} className="px-3 py-3">
                                {f.field_type === 'signature' && val ? (
                                  <img src={val} alt="sig" className="h-8 max-w-16 object-contain rounded border border-gray-100 dark:border-neutral-700 bg-white" />
                                ) : f.field_type === 'date' ? (
                                  <span className="text-gray-700 dark:text-neutral-300 text-xs">{val}</span>
                                ) : (
                                  <span className="text-gray-700 dark:text-neutral-300 text-xs truncate block max-w-[120px]">{val || '-'}</span>
                                )}
                              </td>
                            );
                          })}
                          <td className="px-3 py-3 text-gray-500 text-xs hidden sm:table-cell">{toLocalISO(new Date(sig.created_at))}</td>
                          <td className="px-3 py-3 text-gray-400 text-[10px] hidden lg:table-cell font-mono">{sig.ipv4 || sig.ip_address || '-'}</td>
                          <td className="px-3 py-3 text-right whitespace-nowrap">
                            <div className="flex items-center gap-1 justify-end">
                              <button onClick={() => startEdit(sig)}
                                className="px-2 py-1 text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg text-xs font-medium transition-colors">Edit</button>
                              <button onClick={() => deleteSig(sig.id)}
                                className="px-2 py-1 text-gray-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg text-xs font-medium transition-colors">Delete</button>
                              <button onClick={() => downloadSig(sig.id)} disabled={downloadingId === sig.id}
        ...
        {editSig && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200" onClick={() => setEditSig(null)}>
            <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-black/5 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
              <div className="px-5 py-4 border-b border-gray-100 dark:border-neutral-700 flex items-center justify-between">
                ...
                <button onClick={() => setEditSig(null)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-neutral-300 transition-colors">
              ...
              <div className="p-5 space-y-4 max-h-80 overflow-y-auto">
                {fields.map((f) => {
                  ...
                  return (
                    <div key={f.id}>
                      <label className="block text-xs font-medium text-gray-500 dark:text-neutral-400 mb-1">{f.label}</label>
                      {f.field_type === 'signature' ? (
                        <div className="flex items-center gap-3">
                          {val ? (
                            <img src={val} alt="sig" className="h-10 w-auto object-contain rounded border border-gray-200 dark:border-neutral-700 bg-white" />
                          ) : (
                        ...
                        <input type="text" value={val} onChange={(e) => updateEditValue(f.id, e.target.value)}
              ...
              <div className="px-5 py-4 border-t border-gray-100 dark:border-neutral-700 flex justify-end gap-3">
                <button onClick={() => setEditSig(null)}
                  className="px-4 py-2 text-sm text-gray-600 dark:text-neutral-400 hover:text-gray-800 dark:hover:text-neutral-200 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-700 transition-all">Cancel</button>
                <button onClick={saveEdit}
                  className="px-5 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-all active:scale-[0.98] shadow-sm">Save Changes</button>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Context field modal */}
        {ctxPos && (
          <CtxFieldEditor ctxFieldId={ctxFieldId} ctxNewPos={ctxNewPos} ctxPos={ctxPos} ctxRef={ctxRef}
            fields={fields} numPages={numPages} selectedId={selectedId}
            setCtxPos={setCtxPos} setCtxFieldId={setCtxFieldId} setCtxNewPos={setCtxNewPos}
            updateField={updateField} addField={addField} removeField={removeField} canEdit={canEdit} />
        )}

        {/* Share modal */}
        {showShare && canManage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowShare(false)}>
            <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl w-full max-w-md border border-gray-200 dark:border-neutral-700" onClick={(e) => e.stopPropagation()}>
              <div className="px-5 py-4 border-b border-gray-100 dark:border-neutral-700 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-neutral-100">Share Document</h3>
                <button onClick={() => setShowShare(false)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-neutral-300">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
                  ...
                  <div className="space-y-2">
                    {permissions.map((p) => (
                      <div key={p.id || p.email} className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-neutral-900 rounded-lg">
                        ...
                        <button onClick={() => removeShare(p.email)}
